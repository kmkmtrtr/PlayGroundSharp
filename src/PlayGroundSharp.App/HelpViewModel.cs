...
public sealed partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        [
            ...
            new("CSV / TSV", "ReadCsvAsyncとReadTsvAsyncは先頭行を読み込みます。ヘッダー有無と件数を指定でき、ReadDelimitedAsyncでは任意の区切り文字を使えます。全件保持するReadAll系と逐次処理するStream系も利用できます。", "var csv = await Data.ReadCsvAsync(@\"C:\\data\\items.csv\", hasHeader: true, take: 1000, cancellationToken: ExecutionCancellation);\nvar pipe = await Data.ReadDelimitedAsync(@\"C:\\data\\items.txt\", '|', hasHeader: false, cancellationToken: ExecutionCancellation);"),
    ...
    [
        ...
        [
            ...
            new("CSV / TSV", "ReadCsvAsync and ReadTsvAsync read a bounded leading batch. Configure headers and row count, or use ReadDelimitedAsync for another delimiter. ReadAll and Stream variants support complete and incremental processing.", "var csv = await Data.ReadCsvAsync(@\"C:\\data\\items.csv\", hasHeader: true, take: 1000, cancellationToken: ExecutionCancellation);\nvar pipe = await Data.ReadDelimitedAsync(@\"C:\\data\\items.txt\", '|', hasHeader: false, cancellationToken: ExecutionCancellation);"),
