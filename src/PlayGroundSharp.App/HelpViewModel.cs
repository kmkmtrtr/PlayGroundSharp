...
public sealed partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        [
            ...
            new("JSON", "単一のJSON値をJsonNodeで返します。通常はReadJsonAsyncを使い、大きなトップレベル配列を先頭N件だけ確認するときはReadJsonArrayAsyncを使います。ファイルのドロップ時は全体、先頭1,000件、先頭10,000件から選べます。", "var json = await Data.ReadJsonAsync(@\"C:\\data\\settings.json\", ExecutionCancellation);\nvar rows = await Data.ReadJsonArrayAsync(@\"C:\\data\\large-array.json\", 1000, ExecutionCancellation);"),
            new("JSON Lines", "通常は先頭1,000件をReadJsonLinesAsyncでリストとして読みます。ドロップ時は先頭1,000件、先頭10,000件、全件から選べます。全件をメモリへ載せたくない場合はStreamJsonLinesAsyncで逐次処理してください。", "var rows = await Data.ReadJsonLinesAsync(@\"C:\\data\\events.jsonl\", 1000, ExecutionCancellation);\nvar allRows = await Data.ReadAllJsonLinesAsync(@\"C:\\data\\events.jsonl\", ExecutionCancellation);\n\nawait foreach (var row in Data.StreamJsonLinesAsync(@\"C:\\data\\events.jsonl\", ExecutionCancellation))\n{\n    // 1件ずつ処理\n}")
        ]),
    ...
    [
        ...
        [
            ...
            new("JSON", "ReadJsonAsync returns one JSON value as JsonNode. Use ReadJsonArrayAsync to inspect only the first N values of a large top-level array. File drop offers the complete value, the first 1,000 values, or the first 10,000 values.", "var json = await Data.ReadJsonAsync(@\"C:\\data\\settings.json\", ExecutionCancellation);\nvar rows = await Data.ReadJsonArrayAsync(@\"C:\\data\\large-array.json\", 1000, ExecutionCancellation);"),
            new("JSON Lines", "ReadJsonLinesAsync reads the first 1,000 values by default in generated code. File drop offers 1,000, 10,000, or all values. Use StreamJsonLinesAsync when all values must be processed without retaining them in memory.", "var rows = await Data.ReadJsonLinesAsync(@\"C:\\data\\events.jsonl\", 1000, ExecutionCancellation);\nvar allRows = await Data.ReadAllJsonLinesAsync(@\"C:\\data\\events.jsonl\", ExecutionCancellation);\n\nawait foreach (var row in Data.StreamJsonLinesAsync(@\"C:\\data\\events.jsonl\", ExecutionCancellation))\n{\n    // Process one value\n}")
        ]),
