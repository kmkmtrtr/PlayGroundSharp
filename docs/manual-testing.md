...
| 36 | Execute `new[] { 3, 1, 2 }.Select(async x => { await Task.Delay(x * 100); return x; })` | values appear individually in completion order as `[1] 1`, `[2] 2`, `[0] 3`; another submission cannot start until the sequence finishes |
