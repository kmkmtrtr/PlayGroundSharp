...
public sealed partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        [
            ...
            new("CSV / TSV", "ReadCsvAsync、ReadTsvAsync、ReadDelimitedAsyncは既定で全件を読み込みます。delimiterで任意の区切り文字、hasHeaderでヘッダー有無、takeで先頭件数を指定できます。大きなファイルは全件を保持しないStream系を利用できます。", "var csv = await Data.ReadCsvAsync(@\"C:\\data\\items.csv\", delimiter: ';', hasHeader: true, cancellationToken: ExecutionCancellation);\nvar firstRows = await Data.ReadDelimitedAsync(@\"C:\\data\\items.txt\", '|', hasHeader: false, take: 1000, cancellationToken: ExecutionCancellation);"),
    ...
    [
        ...
        [
            ...
            new("CSV / TSV", "ReadCsvAsync, ReadTsvAsync, and ReadDelimitedAsync read all rows by default. Configure delimiter, hasHeader, or take for a bounded leading batch. Use Stream variants to avoid retaining a large file.", "var csv = await Data.ReadCsvAsync(@\"C:\\data\\items.csv\", delimiter: ';', hasHeader: true, cancellationToken: ExecutionCancellation);\nvar firstRows = await Data.ReadDelimitedAsync(@\"C:\\data\\items.txt\", '|', hasHeader: false, take: 1000, cancellationToken: ExecutionCancellation);"),
