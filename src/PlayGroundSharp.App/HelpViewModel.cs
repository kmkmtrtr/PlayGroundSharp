...
public sealed partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        [
            new("実行結果", "結果行から詳細表示、コピー、保存ができます。名前付きValueTupleはItem1ではなく宣言時の要素名を表示します。Task／ValueTaskの列挙結果は1つの列挙ツリーへまとまり、各要素に元の0始まり位置が［0］のように表示されます。現在の結果はLast、過去の結果はOut[index]で元のオブジェクトを参照できます。"),
    ...
    [
        ...
        [
            new("Results", "A result row can be inspected, copied, or saved. Named ValueTuple elements use their declared names instead of Item1. Enumerated Task and ValueTask results share one sequence tree, and each child shows its original zero-based position as [0], for example. Last refers to the current result, and Out[index] refers to an earlier original result object."),
