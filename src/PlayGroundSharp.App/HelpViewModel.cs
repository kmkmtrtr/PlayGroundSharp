...
public sealed partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        [
            ...
            new("データ型の推論", "変数タブでJSON、オブジェクト、またはオブジェクト列を右クリックし［データ型を推論］を選ぶと、現在値からC#型を生成できます。元の変数は保持され、生成した型付き変数では補完を利用できます。打ち切られたスナップショットでは取得済みの範囲だけを使用します。"),
    ...
    [
        ...
        [
            ...
            new("Data type inference", "Right-click JSON, an object, or a sequence of objects in Variables and choose Infer data type. PlayGroundSharp generates C# models from the current value, retains the original variable, and creates a typed variable with completion. A truncated snapshot uses only its captured portion."),
