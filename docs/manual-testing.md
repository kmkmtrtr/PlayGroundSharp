...
| 47 | Add `System.Net.Http` from the Usings page or with `:using add System.Net.Http`, enter `var client = new HttpClient(); client.`, and open completion | `GetAsync` and `SendAsync` are offered and `new HttpClient().GetType().Name` executes as `HttpClient` |
