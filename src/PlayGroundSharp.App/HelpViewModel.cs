...
public sealed partial class HelpViewModel : ObservableObject
{
    ...
    [
        ...
        [
            ...
            new("JSON", "オブジェクト、配列、スカラーを区別せず1つのJSON値として読み込み、JsonNodeで返します。実体は内容に応じてJsonObject、JsonArray、JsonValueになります。結果はその場で展開でき、ツリーで選択した値はCtrl+C、結果全体は右クリックからコピー、保存、別ウィンドウの詳細表示を利用できます。詳細画面ではCtrl+Fで検索、F3／Shift+F3で一致箇所を移動、Ctrl+Shift+Cで全体コピー、Ctrl+Sで保存できます。検索は総一致数を数えつつ先頭250件だけを描画します。大きな配列やオブジェクトは最初は折りたたまれ、100件単位の範囲から必要な部分だけ展開できます。", "var json = await Data.ReadJsonAsync(@\"C:\\data\\settings.json\");\njson?[\"name\"]"),
            new("JSON Lines", "一行一JSONだけは複数値として区別し、JsonNodeを非同期列挙します。必要な件数でbreakしてください。", "var rows = new List<JsonNode?>();\nawait foreach (var row in Data.ReadJsonLinesAsync(@\"C:\\data\\events.jsonl\"))\n{\n    rows.Add(row);\n    if (rows.Count == 100) break;\n}\nrows")
        ]),
    ...
    [
        ...
        [
            ...
            new("JSON", "ReadJsonAsync treats an object, array, or scalar uniformly as one JSON value and returns JsonNode; the runtime value is a JsonObject, JsonArray, or JsonValue. Expand it inline, press Ctrl+C to copy the selected tree value, or right-click the result to copy, save, or inspect it in a separate window. In the inspector, Ctrl+F searches, F3/Shift+F3 moves between matches, Ctrl+Shift+C copies all, and Ctrl+S saves. Search counts every match while rendering only the first 250. Large structures start collapsed and can be opened in groups of 100 items.", "var json = await Data.ReadJsonAsync(@\"C:\\data\\settings.json\");\njson?[\"name\"]"),
            new("JSON Lines", "Only newline-delimited JSON is treated as multiple values and streamed as JsonNode instances.", "var rows = new List<JsonNode?>();\nawait foreach (var row in Data.ReadJsonLinesAsync(@\"C:\\data\\events.jsonl\"))\n{\n    rows.Add(row);\n    if (rows.Count == 100) break;\n}\nrows")
        ]),
